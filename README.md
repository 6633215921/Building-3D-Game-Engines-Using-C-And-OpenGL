...
### 🌊 The Gerstner Wave 3D Equation (หลายคลื่น — สั้นที่สุด)

ให้จุดเริ่มต้นบนผิวน้ำ
[
\mathbf{P_0}=(x_0,z_0)
]

ตำแหน่งใหม่เมื่อรวม **N คลื่น**
...
[
f_i = k_i(\mathbf{d_i}\cdot\mathbf{P_0} - c_i t)
]
...
[
c_i=\sqrt{\frac{g}{k_i}} ; (\text{deep water})
]
...
* ( \lambda_i ) = wavelength
* ( k_i ) = wave number
* ( \mathbf{d_i} ) = ทิศทางคลื่น (normalize)
* ( c_i ) = ความเร็วคลื่น
* ( Q_i ) = steepness
* ( a_i ) = amplitude
* ( t ) = เวลา
...
**จบครบทุกตัวแปร พร้อมรวมหลายคลื่น — พร้อมแคป** 🌊
